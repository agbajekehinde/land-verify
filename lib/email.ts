...
export async function sendPasswordResetEmail(
  ...
  // Make sure the resetLink uses the app router structure
  const appRouterResetLink = `${process.env.NEXT_PUBLIC_APP_URL}/reset-password/${token}`;
  
  const result = await resend.emails.send({
    from: process.env.EMAIL_FROM || 'support@Landverify.ng',
    to: email,
    subject: "Reset your LandVerify password",
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <img src="${process.env.NEXT_PUBLIC_APP_URL}/LandVerify-logo.png" alt="LandVerify Logo" style="max-width: 200px; margin: 20px 0;">
